...
import { setJobHuntSock, startJobHuntScheduler, runJobHunt } from './job-hunt-agent.js';
...
async function startBot() {
  ...
  sock.ev.on('connection.update', async ({ connection, lastDisconnect }) => {
    ...
    if (connection === 'open') {
      ...
      // Job-hunt agent (job-hunt-agent.js): runs 09:00 + 17:00 Asia/Jerusalem, reports to Roei's DM
      setJobHuntSock(sock, OWNER_JID);
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      if (isOwner && /^חפש משרות/.test(text.trim())) {
        await sock.sendMessage(jid, { text: '🔎 מתחיל ציד משרות... זה לוקח כ-2 דקות — מערכת רואי 🤖' });
        runJobHunt(t => sock.sendMessage(jid, { text: t }), { force: true })
          .catch(e => sock.sendMessage(jid, { text: `⚠️ שגיאה בציד משרות: ${e.message} — מערכת רואי 🤖` }));
        continue;
      }
